...
public class KafkaWorker extends ShutdownableThread {
    ...
    public KafkaWorker(String name, boolean isInterruptible) {
        ...
        topicList = ConfigUtil.getString("topicList");
    ...
    public void doWork() {
        ...
        for (TopicPartition partition : records.partitions()) {
            ...
            for (ConsumerRecord<byte[], byte[]> record : partitionRecords) {
                ...
                String msKey = Base64.getEncoder().encodeToString(record.key());
                KafkaData kafkaData = new KafkaData(record.topic(), msKey, record.value());
