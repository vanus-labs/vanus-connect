...
func (s *whatsappSink) Arrived(ctx context.Context, events ...*ce.Event) cdkgo.Result {
	...
	for _, event := range events {
		...
		userJID := JID{
			User:   data.Info.Sender.User,
			Agent:  data.Info.Sender.Agent,
			Device: data.Info.Sender.Device,
			Server: data.Info.Sender.Server,
			AD:     data.Info.Sender.AD,
		}
		...
		s.client.SendMessage(ctx, types.JID(userJID), message)
