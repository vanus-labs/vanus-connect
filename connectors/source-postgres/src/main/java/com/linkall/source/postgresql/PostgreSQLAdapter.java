...
public class PostgreSQLAdapter implements Adapter1<SourceRecord> {
  ...
  private URI source() {
    return URI.create("vance.debezium.postgresql");
  }

  private String ceId(Struct source) {
    return source.get(AbstractSourceInfo.SCHEMA_NAME_KEY)
        + "."
        + source.get(AbstractSourceInfo.TABLE_NAME_KEY)
        + ":"
        + source.get("lsn").toString()
        + ":"
        + source.get("txId").toString();
  }

  private String ceType(Struct source) {
    return "debezium.postgresql."
        + source.get(AbstractSourceInfo.SCHEMA_NAME_KEY)
        + "."
        + source.get(AbstractSourceInfo.TABLE_NAME_KEY);
  }
  ...
  private void extension(CloudEventBuilder eventBuilder, Struct struct) {
    // source
    Struct source = struct.getStruct(Envelope.FieldName.SOURCE);
    setExtension(eventBuilder, source, PostgreSQLAdapter::adjustExtensionName);
    // op
    ...
    if (transaction != null) {
      setExtension(eventBuilder, transaction, PostgreSQLAdapter::txExtensionName);
    }
  }
